% A_sweep.m
%
% Parameter sweep of solvent_evaporation_sweep_solvent.mph over A [m].
% Runs continue even if individual simulations crash.
%
% s = A * flc2hs(x_prime, epsilon)
% max slope of s = atan(A*15/(16*epsilon)) * 180/pi
% A = 1e-6 -> ~10 deg,  A = 10e-6 -> ~61 deg  (at default epsilon = 5.4e-6 m)
%
% Outputs:
%   Results/A_sweep.mat — r_cell, hh_cell, s_cell, t_end, success
%   Figure 1 — max slope of (hh+s) vs max slope of s, one point per A
...
%% Sweep definition
A_values = linspace(1e-6, 5e-6, 7);   % [m]
n_runs   = length(A_values);
...
for k = 1:n_runs
    A = A_values(k);
    fprintf('[%2d/%2d]  A = %.2e m ... ', k, n_runs, A);

    model.param.set('A', sprintf('%.6e[m]', A));
    ...
    try
        ...
        save(fullfile(results_dir, 'A_sweep.mat'), ...
             'A_values', 'r_cell', 'hh_cell', 's_cell', 't_end', 'success');
    catch ME
        fprintf('FAILED — %s\n', ME.message);
    end
end
...
figure('Name','A Sweep — Max slope','Position',[50 50 700 480]);
scatter(max_slope_s, max_slope_hhs, 80, A_values(idx_ok)*1e6, 'filled');
cb = colorbar;
cb.Label.String = 'A  [µm]';
xlabel('Max slope of s  [°]',    'FontSize', 12);
ylabel('Max slope of hh+s  [°]', 'FontSize', 12);
title('Film surface slope vs substrate slope — A sweep', 'FontSize', 12);
grid on;

for j = 1:numel(idx_ok)
    text(max_slope_s(j), max_slope_hhs(j), ...
         sprintf('  %.1f µm', A_values(idx_ok(j))*1e6), ...
         'FontSize', 8, 'VerticalAlignment', 'middle');
end
saveas(gcf, fullfile(results_dir, 'A_sweep_max_slope.png'));
